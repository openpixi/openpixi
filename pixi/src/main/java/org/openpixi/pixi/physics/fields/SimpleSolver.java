	...
	public void step(Grid g, double tstep) {
		...
		for (int i = 0; i < g.getNumCellsX(); i++) {
			for (int j = 0; j < g.getNumCellsY(); j++) {
				...
				double cz = (g.getEyo(i+1, j) - g.getEyo(i-1, j)) / ( 2 * g.getCellWidth()) -
				...
				double cx = (g.getBzo(i, j+1) - g.getBzo(i, j-1)) / ( 2 * g.getCellHeight());
				double cy = -(g.getBzo(i+1, j) - g.getBzo(i-1, j)) / ( 2 * g.getCellWidth());
