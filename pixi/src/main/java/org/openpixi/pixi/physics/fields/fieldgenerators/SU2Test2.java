...
public class SU2Test2 implements IFieldGenerator {
	...
	public SU2Test2(double[] direction,
					double[] position,
	...
	public void applyFieldConfiguration(Simulation s) {
		...
		for (int ci = 0; ci < numberOfCells; ci++) {
			...
			double electricFieldFactor = -g * as * c * phaseE / Math.pow(sigma, 2.0) *
					...
					(Math.sin(phaseE / Math.pow(sigma, 2.0)) +
							Math.cos(phaseE / Math.pow(sigma, 2.0)) / Math.pow(sigma, 2.0));
			...
			double gaugeFieldFactor = g * as * Math.exp(-Math.pow(phaseU / this.sigma, 2.0) / 2.0) *
					Math.sin(phaseU / Math.pow(sigma, 2.0));
