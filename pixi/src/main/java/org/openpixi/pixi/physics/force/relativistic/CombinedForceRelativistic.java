 ...
 */
public class CombinedForceRelativistic extends ForceRelativistic {

	public CombinedForceRelativistic() {
		super();
	}
	 ...
	 * @param f
	 */
	public void add(ForceRelativistic force) {
		forces.add(force);
	}
	...
	public double getForceX(Particle2D p) {
		double sum = 0;
		for (ForceRelativistic f : forces) {
			sum += f.getForceX(p);
		}
		return sum;
	}
