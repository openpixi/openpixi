...
public class Grid {
	...
	*/
	 ...
	 * This is used for the field equations of motion.
	 * <br>
	 * getTransverseStapleSum(i, d, bd, true) + getTransverseStableSum(i, d, bd, false) == getStapleSum(i, d).
	 * @param index Lattice index
	 * @param d     Direction
	 * @param beamdirection Beam direction
	 * @param calculateTransverseParts true: only purely transverse contributions; false: everything else
	 * @return      Sum of all surrounding staples
	 */
	public GroupElement getTransverseStapleSum(int index, int d, int beamdirection, boolean calculateTransverseParts) {
		GroupElement S = factory.groupZero();
		int ci1 = shift(index, d, 1);
		int ci2, ci3, ci4;
		for (int i = 0; i < numDim; i++) {
			boolean inTransversePlane = (i != beamdirection) && (d != beamdirection);
			if ((i != d) && (inTransversePlane == calculateTransverseParts)) {
				ci2 = shift(index, i, 1);
				ci3 = shift(ci1, i, -1);
				ci4 = shift(index, i, -1);
				GroupElement U1 = getU(ci1, i).mult(getU(ci2, d).adj());
	 ...
	 * but takes into account only those plaquettes transverse to a certain beam direction.
	 * This is used for the field equations of motion.
	 * Calculation is based on Unext instead of U.
	 * <br>
	 * getTransverseStapleSum(i, d, bd, true) + getTransverseStableSum(i, d, bd, false) == getStapleSum(i, d).
	 * @param index Lattice index
	 * @param d     Direction
	 ...
	 * @return      Sum of all surrounding staples
	 */
	public GroupElement getTransverseStapleSumNext(int index, int d, int beamdirection, boolean calculateTransverseParts) {
		GroupElement S = factory.groupZero();
		int ci1 = shift(index, d, 1);
		int ci2, ci3, ci4;
		for (int i = 0; i < numDim; i++) {
			...
			if ((i != d) && (inTransversePlane == calculateTransverseParts)) {
				...
				ci3 = shift(ci1, i, -1);
				ci4 = shift(index, i, -1);
				GroupElement U1 = getUnext(ci1, i).mult(getUnext(ci2, d).adj());
				U1.multAssign(getUnext(index, i).adj());
				GroupElement U2 = getUnext(ci4, d).mult(getUnext(ci3, i));
				U2.adjAssign();
				U2.multAssign(getUnext(ci4, i));
				double areaFactor = 1.0 / Math.pow(getLatticeSpacing(i), 2);
				U1.addAssign(U2);
				U1.multAssign(areaFactor);
				S.addAssign(U1);
