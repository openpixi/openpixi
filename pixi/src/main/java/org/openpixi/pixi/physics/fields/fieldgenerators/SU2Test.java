...
/* SU2Test based on SU2FocusedGaussianPulse */

public class SU2Test implements IFieldGenerator {
	...
	public SU2Test(double[] direction,
	...
	public void applyFieldConfiguration(Simulation s) {
		...
		/*
			Cycle through each cell and apply the test configuration to the links and electric fields.
		 */
		for (int ci = 0; ci < numberOfCells; ci++) {
			int[] cellPosition = grid.getCellPos(ci);
			double[] currentPosition = getPosition(cellPosition);

			// Set origin to focal point of the test pulse.
			double[] pos = new double[this.numberOfDimensions];
			for (int i = 0; i < this.numberOfDimensions; i++) {
				pos[i] = currentPosition[i] - this.position[i];
			}

			double ratio = Math.sqrt(1 - this.eccentricity * this.eccentricity);
			ratio = 1 / ratio;

			// Multiplicative factor for the focused gaussian pulse at t = 0 (for electric fields)
			double electricFieldFactor = g * as * c * pulseFunction(pos, 0, true);

			// Multiplicative factor for the focused gaussian pulse at t = dt/2 (for links)
			double gaugeFieldFactor = g * as * pulseFunction(pos, this.timeStep / 2.0, false);
	...
	private double pulseFunction(double[] pos, double t, boolean derivative) {
		double parallel = 0;
		double mod = 0;
		double semiminor = Math.sqrt(1- this.eccentricity * this.eccentricity) * this.size;

		for (int i = 0; i < pos.length; i++) {
			parallel += pos[i] * this.direction[i];
			mod += pos[i] * pos[i];
		}
		double orthogonal = mod - parallel * parallel;
		orthogonal = Math.sqrt(orthogonal);
		mod = Math.sqrt(mod);

		parallel += t * this.s.getSpeedOfLight();

		double ellipse = orthogonal * orthogonal / (this.size * this.size);
		if (ellipse >= 1) {
			return 0;
		}
		double height = semiminor * Math.sqrt(1 - ellipse);

		if (derivative) {
			return gaussian(parallel, 0, height) * parallel / height;
		}
		else {
			return height * gaussian(parallel, 0, height);
		}
	}
