 ...
 * MODIFICATION:
 *
 * Show min/max axis labels if no axis labels would be drawn otherwise.
 */
...
public class MyAxisScalePolicyAutomaticBestFit extends
		AxisScalePolicyAutomaticBestFit {
	/**
	 * Internally used for rounding to ticks, calculated once per paint
	 * iteration.
	 */
	...
	public List<LabeledValue> getScaleValues(final Graphics g2d,
			final IAxis<?> axis) {
		final double labelspacepx = axis.getAccessor()
				.getMinimumValueDistanceForLabels(g2d);
		final double formattingspace = axis.getFormatter()
	 ...
	 *
	 * @param resolution
	 *            the distance in the value domain of the chart that has to be
	 *            at least between to labels.
	 *
	...
	protected List<LabeledValue> getLabels(final double resolution,
		...
		if (resolution > 0) {
			...
			final double tickResolution = this.roundToTicks(resolution, false,
					false, axis).getValue();
			double value = Math.ceil(min / tickResolution) * tickResolution;
			// This was value before the patch that prevents the labels from
			// jumping:
			// It's benefit was that the first label was not this
			// far from the start of data (in case startMajorTicks of axis is
			// true):
			// double value = min;
			...
			while ((value <= max) && (loopStop < 100)) {
				if (loopStop == 99) {
					if (AAxis.DEBUG) {
						System.out.println(axis.getAccessor().toString()
								+ " axis: loop to high");
					}
				}
				...
				label = this.roundToTicks(value, false, !firstMajorFound
						&& axis.isStartMajorTick(), axis);
				...
				loopStop++;
				if (firstMajorFound || !axis.isStartMajorTick()
						|| label.isMajorTick()) {
					...
					if ((value <= max) && (value >= min)) {
						...
						if (AAxis.DEBUG) {
							System.out.println("Dropping label (too high) : ("
									+ label + ")[max: " + max + "]");
						}
					} else if (value < min) {
						if (AAxis.DEBUG) {
							System.out.println("Dropping label (too low) : ("
									+ label + ")[min: " + min + "]");
						}
					}
				}
			...
			if (collect.size() < 2) {
				...
				label.setValue((label.getValue() - min)
						/ range);
				collect.add(label);
	 ...
	 * @param findMajorTick
	 *            if true the returned value will be a major tick (which might
	 *            be fare more away from the given value than the next major
	 *            tick).
	 * @return the value rounded to minor or major ticks.
	 */
	@Override
	protected LabeledValue roundToTicks(final double value,
			final boolean floor, final boolean findMajorTick,
			final IAxis<?> axis) {
