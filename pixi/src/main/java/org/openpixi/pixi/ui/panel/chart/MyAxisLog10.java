 ...
 *    if (toTransform < 1)
 * It is possible to relax this condition to
 *    if (toTransform < 0)
 */
 ...
 */
public class MyAxisLog10<T extends AxisScalePolicyTransformation> extends
		AAxisTransformation<T> {

	/** Generated <code>serialVersionUID</code>. */
	private static final long serialVersionUID = -1783172443489534956L;

	/**
	 * Creates an instance that uses a {@link LabelFormatterSimple} for
	 * formatting numbers and a {@link AxisScalePolicyAutomaticBestFit} for
	...
	public MyAxisLog10() {
		/*
		 * I consider this necessary cast to T as a bug (compare with
		 * declaration if T in class header):
		 */
		this(new LabelFormatterSimple(),
				(T) new AxisScalePolicyTransformation());
	}
	 ...
	 *            implementations like {@link LabelFormatterSimple}, a log axis
	 *            is complicated enough to understand.
	 *
	 * @param scalePolicy
	 *            controls the ticks/labels and their distance.
	 */
	public MyAxisLog10(final IAxisLabelFormatter formatter, final T scalePolicy) {
	...
	public double transform(final double in) {
		...
		if (toTransform == 0) {
			...
			if (this.m_accessor == null) {
				throw new IllegalStateException("Connect this axis ("
						+ this.getAxisTitle().getTitle()
						+ ") to a chart first before doing this operation.");
			}
			Iterator<ITrace2D> itTraces = this.m_accessor.getChart()
					.getTraces().iterator();
			if (!itTraces.hasNext()) {
				toTransform = 1.0;
			} else {
				...
				while (itTraces.hasNext()) {
					...
					if (trace.iterator().hasNext()) {
						// Illegal value for transformation defined by a point
						// added:
