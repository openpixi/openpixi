...
import org.openpixi.pixi.physics.fields.currentgenerators.SU2LorenzLightConeGaussPulseCurrent;
 ...
 */
public class YamlSU2LorenzLightConeGaussPulseCurrent {
	...
	public boolean checkConsistency(Settings settings) {
		if (direction >= settings.getNumberOfDimensions()) {
			System.out.println("SU2LorenzLightConeGaussPulseCurrent: direction index exceeds the dimensions of the system.");
			return false;
		}

		if (location.size() != settings.getNumberOfDimensions()) {
			System.out.println("SU2LorenzLightConeGaussPulseCurrent: location vector does not have the right dimensions.");
			return false;
		}

		int numberOfComponents = settings.getNumberOfColors() * settings.getNumberOfColors() - 1;
		if (aColor.size() != numberOfComponents) {
			System.out.println("SU2LorenzLightConeGaussPulseCurrent: aColor vector does not have the right dimensions.");
			return false;
		}

		if (v == 0) {
			System.out.println("SU2LorenzLightConeGaussPulseCurrent: Orientation has to be non-zero.");
			return false;
		}

		if (width <= 0) {
			System.out.println("SU2LorenzLightConeGaussPulseCurrent: Width has to be a non-zero positive number.");
			return false;
		}
		return true;
	}
	...
	/**
	 * Returns an instance of SU2LorenzLightConeGaussPulseCurrent according to the parameters in the YAML file.
	 *
	 * @return Instance of SU2LorenzLightConeGaussPulseCurrent.
	 */
	public SU2LorenzLightConeGaussPulseCurrent getCurrentGenerator() {
		...
		return new SU2LorenzLightConeGaussPulseCurrent(direction, locationArray, width, aColorArray, a, v);
	}
}
